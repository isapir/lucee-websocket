...
public class ConnectionManager implements lucee.runtime.type.Objects {
    ...
    static ConcurrentHashMap<String, ConnectionManager> connectionManagers = new ConcurrentHashMap();   // different connection manager for each endpoint
    ...
    private ConnectionManager(String id){
        ...
        if (!availableLoggers.contains("websocket")){
            ...
            luceeApp.log(Log.LEVEL_WARN, "WebSocket logs are disabled. To enable logging please define a logger named [websocket] in the Web Admin.", "websocket", "application");
        }
        else {
    ...
    public int subscribe(String chanId, WebSocket websocket){
        ...
        WebsocketUtil.invokeListenerMethodWithNamedArgs(
                 this.appListener
                ,LISTENER_METHOD_ON_SUBSCRIBE
                ,"channel", chanId
                ,"subscribers", subs
                ,"websocket", websocket
    ...
    public int unsubscribe(String chanId, WebSocket websocket){
        ...
        if (channel != null){
            ...
            WebsocketUtil.invokeListenerMethodWithNamedArgs(
                     this.appListener
                    ,LISTENER_METHOD_ON_UNSUBSCRIBE
                    ,"channel", chanId
                    ,"subscribers", subs
                    ,"websocket", websocket
                    ,"connectionManager", this
    ...
    /**
     * unsubscribe the connection from all of the channels to which it is subscribed by calling unsubscribe() for each chanel id
     *
     * @param websocket
     */
    public void unsubscribeAll(WebSocket websocket){

        // iterate over getChannelNames() which returns a copy and not getChannels() which is a reference to the actual list so that we don't get a ConcurrentModification error
    ...
    public synchronized void removeChannel(String chanId){
        ...
        if (channel != null){
            ...
            WebsocketUtil.invokeListenerMethodWithNamedArgs(
                     this.appListener
                    ,LISTENER_METHOD_ON_CHANNEL_CLOSE
                    ,"channel", chanId
                    ,"connectionManager", this
            );
        ...
        if (channel == null && doCreate){
            ...
            synchronized (this){
                ...
                if (channel == null){
                    ...
                    WebsocketUtil.invokeListenerMethodWithNamedArgs(
                             this.appListener
                            ,LISTENER_METHOD_ON_CHANNEL_OPEN
                            ,"channel", chanId
                            ,"connectionManager", this
                    );
