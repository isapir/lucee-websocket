...
public class ServerEndpoint extends Endpoint {
	 ...
	 * @param endpointConfig The configuration that was set in HandshakeHandler.modifyHandshake(), which provides access to the
	...
	public void onOpen(Session wsSession, EndpointConfig endpointConfig) {
		...
		String wsId = websocket.getId();

		connMgr.log(Log.LEVEL_DEBUG, "connection " + wsId + " enter onOpen()");

		Struct struct = WebsocketUtil.getStruct(websocket);
		String channel = null;
		String k, v;

		struct.setEL(WebsocketUtil.KEY_WEBSOCKET_ID, wsId);

		Struct pathParams = LuceeAppsUtil.getCreationUtil().createStruct();
		for (Map.Entry<String, String> e : websocket.getPathParameters().entrySet()) {
			k = e.getKey();
			v = e.getValue();
			pathParams.setEL(LuceeAppsUtil.toKey(k), v);
		...
		connMgr.log(Log.LEVEL_DEBUG, "connection " + wsId + " calling listener.onOpen()");
		...
		if (LuceeAppsUtil.isBooleanFalse(luceeResult)) {
			...
			connMgr.log(Log.LEVEL_INFO, "connection " + wsId
		...
		wsSession.addMessageHandler(new MessageHandler.Whole<String>() {
			...
			public void onMessage(String message) {
				...
				connMgr.log(Log.LEVEL_DEBUG, "connection " + wsId + " enter onMessage(); " + message);
