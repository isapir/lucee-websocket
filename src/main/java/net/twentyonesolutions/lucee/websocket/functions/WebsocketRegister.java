...
import net.twentyonesolutions.lucee.websocket.HandshakeHandlerWithHost;
 ...
 */
public class WebsocketRegister extends BIF {
...
//        System.out.println(args);
        ...
        if (!LuceeApps.getDecisionUtil().isSimpleValue(args[0]))
            throw new RuntimeException("Invalid argument, expected a String as the first argument for " + getClass().getSimpleName() + "()");

        if (!LuceeApps.getDecisionUtil().isComponent(args[1]))
            throw new RuntimeException("Invalid argument, expected a Component as the second argument for " + getClass().getSimpleName() + "()");

//        if (args.length > 2){
//
//            if (!LuceeApps.getDecisionUtil().isStruct(args[2]))
//                throw new RuntimeException("Invalid argument, expected a Struct as the third argument for " + getClass().getSimpleName() + "()");
//
//        }
        ...
        String httpHost = (String)pc.cgiScope().get(LuceeApps.toKey("HTTP_HOST"));
        boolean hasHttpHost = endpoint.regionMatches(true, 0, httpHost, 0, httpHost.length());

        if (hasHttpHost){

            endpoint = endpoint.substring(httpHost.length());
        ...
        try {
            ...
            Configurator.configureEndpoint(
                     endpoint
                    ,LuceeEndpoint.class
                    ,hasHttpHost ? HandshakeHandlerWithHost.class : HandshakeHandler.class
                    ,luceeApp
            );
        }
        catch (Throwable t){
