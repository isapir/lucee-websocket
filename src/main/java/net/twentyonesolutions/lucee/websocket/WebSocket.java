...
public class WebSocket implements javax.websocket.Session, Objects {
	...
	public WebSocket(Session websocketSession) {
		...
		// this.struct =
		// (Struct)websocketSession.getUserProperties().get(HandshakeHandler.WEBSOCKET_METHOD_LUCEE_STRUCT); // created
		// and set during handshake
		 ...
		 * this.struct.put(WEBSOCKET_METHOD_PATH_PARAMETERS, pathParams); for (Map.Entry<String, String> e :
		 * wsSession.getPathParameters().entrySet()){ pathParams.setEL(e.getKey(), e.getValue()); } //
		 */

		/*
		 * Map<String, String> pathParameters = wsSession.getPathParameters(); Iterator it =
		 * pathParameters.entrySet().iterator(); while (it.hasNext()){
		 *
		 * Map.Entry e = (Map.Entry) it.next(); pathParams.setEL(LuceeApps.toKey(e.getKey().toString()), e.getValue());
		 * } //
		 */

		/*
		 * Set<Session> openSessions = this.wsSession.getOpenSessions(); Array arr =
		 * LuceeApps.getCreationUtil().createArray(); Iterator<Session> it = openSessions.iterator(); while
		 * (it.hasNext()){
		 *
		 * try { arr.append(it.next().getId()); } catch (PageException e) { e.printStackTrace(); } }
		 *
	 ...
	 * {@link #addMessageHandler(Class, MessageHandler.Partial)} or
	 ...
	 *
	 * @param timeout
	 *            The new idle timeout for this session in milliseconds. Zero or negative values indicate an infinite
	 *            timeout.
	 */
	@Override
	...
	/**
	 * Close the connection to the remote end point using the code {@link CloseReason.CloseCodes#NORMAL_CLOSURE} and an
	 * empty reason phrase.
	 *
	 * @throws IOException
	 ...
	 *
	 * @param closeReason
	 *            The reason the WebSocket session is being closed.
	 * @throws IOException
	 *             if an I/O error occurs while the WebSocket session is being closed.
	 */
	@Override
	 ...
	 * *
	 *
	 * @return a new List with the names of the channels to which this connection is subscribed / public List<String>
	 * getChannelNames(){
	 *
	 * List<String> result = new ArrayList( (Set)wsSession.getUserProperties().get("channels") ); return result; } //
	 */
		...
		if (WEBSOCKET_METHOD_CLOSE.equals(method)) {
			...
			try {
				...
				if (args.length > 0) {
					...
					this.close(new CloseReason(
							CloseReason.CloseCodes.getCloseCode(LuceeApps.getCastUtil().toIntValue(args[0])), reason));
				}
				else {
